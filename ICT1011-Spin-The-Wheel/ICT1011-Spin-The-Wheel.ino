...
// Sector k is centered at top when wheelAngle = -(k - 0.5) * step
static inline float thetaForSectorAtTop(uint8_t k, uint8_t N){
  return -((float)k - 0.5f) * stepSize(N);
}

// Right-open boundary, full wedge ownership
...
void clearScreen(uint16_t col){
    // Optimization: Only clear the wheel area instead of the whole screen
    // display.clearScreen(); // OLD SLOW WAY
    display.drawRect(0,0,SCR_W,SCR_H, 1, col); // Slightly faster fill if supported
}
...
void handleButtons(){
  ...
  if(m & (BTN_LOWER_LEFT | BTN_LOWER_RIGHT)){
    if(!latch){ latch = true; if(!spinning) beginSpin(); else skipSpin=true; }
  } else { latch = false; }
  btn.last = m;
}
