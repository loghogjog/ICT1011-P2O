...
// Menu state
DiceAppState diceCurrentState = MENU;
...
void runDecisionDice(TinyScreen &display, bool &exitToMenu) {
  ...
  if (diceCurrentState == MENU) {
    handleMenuInput(currentButtonState, exitToMenu);
  } else if (diceCurrentState == ROLLING) {
